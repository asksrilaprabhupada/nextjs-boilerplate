...
npm run dev
npm run build
npm test          # vitest — fusion weighting, junk floor, dedup, chunk rules, tiering, snippets, planners, verbatim validator, rerank-failure honesty
...
(one schema-constrained query plan — and nothing else; the article/organizing
call is deleted), Cohere Rerank v4.0 Pro (relevance reranking, judged against
the original question only). No model writes a word a devotee reads.
...
│   │   ├── results/
│   │   │   ├── 01-narrative-response.tsx (prints results.passages first-to-last with nothing above them: label, words, citation, copy; Essay | By source views of the same list; collapsed "N more passages" citation tier below)
...
│   │   │   ├── rerank.ts              (ONE Cohere call over the whole pool, whole ranking back, defensive 1,000 ceiling)
│   │   │   ├── chunk-dedupe.ts        (the two id rules: no chunk beside its verse, one chunk per verse)
│   │   │   ├── select.ts              (pins lead, first 20 are main, every other survivor is Dig Deeper)
...
│   │   │   ├── render.ts             (the letter context notice — all that is left of the deleted article renderer)
