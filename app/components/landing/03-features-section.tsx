...
const features = [
  {
    ...
    color: "var(--accent)", bgTint: "color-mix(in srgb, var(--accent) 10%, transparent)",
  ...
  },
  {
    ...
    color: "var(--accent)", bgTint: "color-mix(in srgb, var(--accent) 10%, transparent)",
    title: "36 Books, 3,700 Lectures, 6,500 Letters",
    description: "Bhagavad Gītā, Śrīmad Bhāgavatam, Caitanya Caritāmṛta, plus 33 more books, thousands of recorded lectures, and personal letters — all searchable from one interface.",
    span: true,
  },
  {
...
export default function FeaturesSection() {
  ...
  return (
    <section ref={ref} style={{ padding: "clamp(48px, 8vw, 100px) clamp(16px,5vw,80px)", maxWidth: 1200, margin: "0 auto", background: "linear-gradient(145deg, color-mix(in srgb, var(--surface-raised) 62%, transparent), color-mix(in srgb, var(--accent-tint) 56%, transparent) 55%, color-mix(in srgb, var(--p-gold) 50%, transparent))", borderRadius: "clamp(18px, 3vw, 28px)", border: "1px solid color-mix(in srgb, var(--surface-raised) 65%, transparent)", boxShadow: "0 18px 44px color-mix(in srgb, var(--ink) 10%, transparent)" }}>
      <div className="scroll-reveal" style={{ textAlign: "center", marginBottom: 60 }}>
        <p className="section-label">Features</p>
        <h2 className="font-display" style={{ fontSize: "clamp(32px,4vw,52px)", fontWeight: 600, lineHeight: 1.15, letterSpacing: "-0.02em", color: "var(--ink)" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
        {features.map((f, i) => (
          <div key={f.title} className="aurora-card scroll-reveal" style={{ gridColumn: f.span ? "span 2" : "span 1", transitionDelay: `${i * 80}ms` }}>
            ...
            <h3 className="font-body" style={{ fontSize: 19, fontWeight: 500, color: "var(--ink)", marginBottom: 10 }}>{f.title}</h3>
            <p className="font-body" style={{ fontSize: 16, fontWeight: 400, lineHeight: 1.7, color: "var(--ink)" }}>{f.description}</p>
          </div>
        ))}
      </div>
      <style jsx>{`
