...
const cards = [
  ...
  },
  {
    ...
    description: "36 books, 3,700 lectures, and 6,500 letters — Bhagavad Gītā to personal correspondence — searched together, instantly.",
    color: "var(--accent)",
  },
];
      ...
      <div className="why-different-grid" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 20 }}>
        {cards.map((card) => (
          <div key={card.title} className="aurora-card scroll-reveal" style={{ padding: "28px 24px" }}>
            <div style={{ width: 44, height: 44, borderRadius: 12, background: `color-mix(in srgb, var(--accent) 8%, transparent)`, display: "flex", alignItems: "center", justifyContent: "center", color: card.color, marginBottom: 16 }}>
              {card.icon}
            </div>
            <h3 className="font-body" style={{ fontSize: 18, fontWeight: 600, color: "var(--ink)", marginBottom: 8 }}>{card.title}</h3>
            <p className="font-body" style={{ fontSize: 15, fontWeight: 400, lineHeight: 1.7, color: "var(--ink-muted)" }}>{card.description}</p>
          </div>
        ))}
      </div>
