...
export default function FeatureRequestOverlay() {
  ...
  if (sent) {
    return (
      <div style={{ textAlign: "center", padding: "40px 0" }}>
        ...
        <h2 className="font-display" style={{ fontSize: "1.4rem", fontWeight: 600, color: "var(--ink)", marginBottom: 8 }}>Feature request received!</h2>
        <p className="font-body" style={{ fontSize: 15, color: "var(--ink-muted)" }}>We appreciate your ideas. Hare Kṛṣṇa!</p>
  ...
  return (
    <div>
      <h2 className="font-display" style={{ fontSize: "1.6rem", fontWeight: 600, color: "var(--ink)", marginBottom: 8, letterSpacing: "-0.02em" }}>Submit a Feature Request</h2>
      <p className="font-body" style={{ fontSize: 14, color: "var(--ink-muted)", marginBottom: 20 }}>
        Help us make Ask Śrīla Prabhupāda even better. What would you love to see?
      </p>
      <form onSubmit={handleSubmit} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
        <div>
          <label className="font-body" style={{ fontSize: 12, fontWeight: 500, color: "var(--ink-muted)", marginBottom: 6, display: "block", textTransform: "uppercase", letterSpacing: "0.1em" }}>Your Name</label>
          <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Your name" style={inputStyle}
            onFocus={e => { e.currentTarget.style.borderColor = "var(--accent)"; }} onBlur={e => { e.currentTarget.style.borderColor = "color-mix(in srgb, var(--accent-tint) 30%, transparent)"; }} />
        </div>
        <div>
          <label className="font-body" style={{ fontSize: 12, fontWeight: 500, color: "var(--ink-muted)", marginBottom: 6, display: "block", textTransform: "uppercase", letterSpacing: "0.1em" }}>Email (optional)</label>
          <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="your@email.com" style={inputStyle}
            onFocus={e => { e.currentTarget.style.borderColor = "var(--accent)"; }} onBlur={e => { e.currentTarget.style.borderColor = "color-mix(in srgb, var(--accent-tint) 30%, transparent)"; }} />
        </div>
        <div>
          <label className="font-body" style={{ fontSize: 12, fontWeight: 500, color: "var(--ink-muted)", marginBottom: 6, display: "block", textTransform: "uppercase", letterSpacing: "0.1em" }}>Feature Description</label>
